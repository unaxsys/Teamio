...
- [x] Бордове (локален CRUD: създаване/преименуване/изтриване и превключване).
- [x] Колони с подреждане и персистентност (локално за всеки борд).
- [~] Карти: UI drag & drop работи; backend `PATCH /cards/:id/move` остава за следваща стъпка.
- [x] Покани по имейл (MVP локален flow по борд с роля и срок).
